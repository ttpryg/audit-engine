...
class FileAuditStorage implements AuditRepositoryInterface
{
    ...
    {
        $logs = $this->readAllLogs();
        $filtered = array_filter($logs, fn (AuditLog $log) => $log->getEntityType() === strtolower($entityType) && (string) $log->getEntityId() === (string) $entityId);
    ...
    {
        $logs = $this->readAllLogs();
        $filtered = array_filter($logs, fn (AuditLog $log) => (string) $log->getActorId() === (string) $actorId);
    ...
    {
        $logs = $this->readAllLogs();
        $filtered = array_filter($logs, fn (AuditLog $log) => $log->getEventName() === $eventName);
